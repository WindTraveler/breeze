...
function deleteTemplate(item,tpl){
  ...
  var curr_tpl = '<?php echo $this->_var['curr_template']['code']; ?>';
  if (curr_tpl == tpl) {
    return false;
  }
  if (confirm('<?php echo $this->_var['lang']['del_template_confirm']; ?>')){
    Ajax.call('template.php?is_ajax=1&act=delete', 'tpl_name=' + tpl, function(data){
      if (data.error == 0) {
        alert('<?php echo $this->_var['lang']['del_success']; ?>');
      } else {
        alert('<?php echo $this->_var['lang']['del_fail']; ?>');
      }
      console.log(data);
      par.remove();
